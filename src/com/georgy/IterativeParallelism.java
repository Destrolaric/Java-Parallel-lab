...
public class IterativeParallelism implements ScalarIP {
    ...
    private <T,R> List<R> runThreads(int threads, List<? extends T> values, Function<List<? extends T>, R> function) throws InterruptedException {
        List<List<? extends T>> jobsList = splitList(threads, values);
        if (mapper!=null){
            return mapper.map(function,jobsList);
        }
        List<workList<T, R>> workers = jobsList.stream().map(val -> new workList<>(function, val)).collect(Collectors.toList());
